        ...
        for suite_id in suite_ids:
            ...
            for test_case in r:
                ...
                try:
                    ...
                    self.logger.info(f"Response for test case {tc_full_ext_id} added to test plan: {response}")

                except TLResponseError as e:
                    if "3045" in str(e):
                        self.logger.info(f"Test case {tc_full_ext_id} is already linked to the test plan '{inputs.PLAN_NAME}' for platform ID {plt_id}. Skipping.")
                    else:
                        raise
